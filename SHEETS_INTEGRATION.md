...
The sample below supports an optional secret token and deduplicates by entry `id` to avoid duplicate rows.
...
2. Choose **"Web app"**.
3. For **"Execute as"** choose **"Me"** (your account). For **"Who has access"** choose **"Anyone"** or **"Anyone, even anonymous"** depending on your needs. If you set `EXPECTED_TOKEN`, you can safely use a more open access setting since the script enforces the token.
...
- Replace `SPREADSHEET_ID` in the script with the value from your sheet URL (the long ID string).
...
- Add dedupe-by-id logic on the client side too (to avoid sending duplicates). Currently dedupe is handled server-side in the sample script.
